...
class ConvertDate {
  ...
  static String getDateOfLastMsg(
    ...
    if (now.year == sent.year) {
      if (now.month == sent.month) {
        ...
        else return addZero(sent,false);
      }
      else
  ...
  static String addZero(DateTime time, bool year){
    String date = '${time.day < 10 ? '0${time.day}' : time.day.toString()}.${time.month < 10 ? '0${time.month}' : time.month.toString()}';
    if(year) {
  ...
  static String getLastActiveTime({required BuildContext context, required String lastActive}){
    ...
    print('DateTime: ${time.day}.${time.month}.${time.year} ${time.hour}:${time.minute}:${time.second}.${time.millisecond}');
